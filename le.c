...
void move_line(char command[10][20], int num)
{
    ...
    if (num == 2 && isNumber(command[1]) && isNumber(command[2]))
    {
        ...
        struct line *temp = start, *move, *put;
        for (k = n; k > 1; --k)
            temp = temp->next_line;
        move = temp;
        temp = start;
        ...
        //deletion
        if (move == start)
            start = move->next_line;
        if (move->next_line != NULL)
            move->next_line->prev_line = move->prev_line;
        if (move->prev_line != NULL)
            move->prev_line->next_line = move->next_line;
        //same as copying
        if (temp == start)
        {
